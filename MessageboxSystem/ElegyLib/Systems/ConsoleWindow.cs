...
namespace EquestriEngine.Systems
{
    ...
    {
        private SingleColorObject _windowTexture;
        ...
        public ConsoleWindow(object game)
            : base(game)
        {
            _font = new FontObject("{console}", @"fonts\celestia_redux");
        ...
        {
            _windowTexture = new SingleColorObject("{console}", Color.White);
            _renderedText = new TargetObject("{rendred_text}", EquestriEngine.Settings.WindowWidth, 256);
            base.LoadContent();
        }
            ...
            if (Console_Open)
            {
                if (_currentMode == ConsoleMode.Stats)
                {
                    ...
                    if (refresh > 1.0)
                    {
                        ...
                        {
                          frames,
                          ((int)System.Environment.WorkingSet / 1048576),
                          AssetManager.ItemsLoaded
                        });
